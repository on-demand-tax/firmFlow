...
> **참고:** MVP 구현 당시(2026-06-20) 설계 히스토리입니다. **현재 상태·스키마·API는 [`docs/specification.md`](../../specification.md) (특히 §6)가 정본**입니다.  
> 프로젝트 유형 enum은 [`2026-06-20-project-types-design.md`](2026-06-20-project-types-design.md)가 아니라 **§3.3 + `lib/project-types.ts`** 를 따르세요.
...
**확장성 (Client Profile):** MVP는 시간·경비·Drive 연동에 필요한 최소 필드만 포함한다. MongoDB + Mongoose 특성상 **향후 필드 추가가 가능**하며, 기존 문서는 새 필드 없이도 정상 동작한다. 추가 시 `required: false`와 기본값으로 **점진적 마이그레이션**한다.

| 확장 방식 | 용도 |
|-----------|------|
| 스키마 필드 추가 | 정형 데이터 (연락처, 주소, 업종, 담당 회계사 등) |
| `notes` / `metadata` | 아직 정하지 않았거나 자주 바뀌는 항목 |
| 별도 `ClientProfile` 문서 (1:1) | 필드가 많아질 때 Client `_id` 참조 |

Drive 폴더명은 **`clientCode` 기준**이므로, 이후 `name` 변경만으로는 폴더 경로와 충돌하지 않는다.

**Phase 2 후보 필드 (예시, MVP 미포함):** `phone`, `email`, `address`, `industry`, `assignedAccountantId`, `contractType`, `tags`

### 5.3 Project (`models/Project.ts`)

**확장 명세:** `docs/superpowers/specs/2026-06-20-project-types-design.md` — 9종 `projectType`, `billingModel` 메타데이터, 동적 등록 폼. MVP는 메타데이터 + 시간/경비 추적만 (자동 청구는 Phase 2).

`Completed`는 Preparer 드롭다운에서 제외.
